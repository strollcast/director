...
export const POST: APIRoute = async (context) => {
  ...
  try {
    const response = await fetch(`${apiUrl}/admin/fix-file-mismatches`, {
      method: 'POST',
      headers: {
    ...
    console.error('Error fixing file mismatches:', error);
    return new Response(JSON.stringify({ error: 'Failed to fix file mismatches' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
